...
def _AddSuffixes(srcs, suffixes):
    ret = []
    for suffix in suffixes:
        ret += [s[:-len(".proto")] + suffix for s in srcs]
    return ret

def _CcOuts(srcs, use_grpc_plugin = False, extra_suffixes = []):
    suffixes = [".pb.cc", ".pb.h"] + extra_suffixes
    if use_grpc_plugin:
        suffixes += [".grpc.pb.cc", ".grpc.pb.h"]
    return _AddSuffixes(srcs, suffixes)
    ...
    for src in srcs:
        ...
        if ctx.attr.gen_cc:
            ...
            outs.extend(_CcOuts(
                ...
                extra_suffixes = ctx.attr.extra_cc_suffixes,
            ))
        if ctx.attr.gen_py:
...
proto_gen = rule(
    attrs = {
        ...
        "gen_cc": attr.bool(),
        "gen_py": attr.bool(),
        "extra_cc_suffixes": attr.string_list(),
...
Args:
  ...
  extra_cc_suffixes: a list of suffices (e.g. ".foo.h", ".foo.cc") for files
    generated by the enabled plugin.
...
def cc_proto_library(
    ...
    outs = _CcOuts(srcs, use_grpc_plugin = use_grpc_plugin)
    gen_srcs = [p for p in outs if p.endswith(".cc")]
    gen_hdrs = [p for p in outs if p.endswith(".h")]
